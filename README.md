...
### 동영상의 하이라이트를 판단해주고 클립까지 생성해주는 프로그램입니다.
...
## 📖프로젝트 개요
최근 인터넷 방송 산업이 폭발적인 성장세를 보이며, 트위치·유튜브 등을 중심으로 다양한 1인 미디어 콘텐츠가 제작되고 있습니다. 이에 따라 방송 VOD를 클립 형식으로 편집하여 하이라이트 영상으로 재가공하는 수요도 함께 증가하고 있으며, 관련된 영상 편집 작업의 자동화에 대한 관심도 높아지고 있습니다.

본 프로젝트는 이러한 흐름에 맞춰, 게임방송 및 인터넷 방송 영상을 대상으로 음성 인식 기반 하이라이트 추출 기능을 제공하는 데스크탑 응용프로그램입니다.
영상 속 음성을 텍스트로 전환한 뒤, 의미 있는 발화나 감정 표현이 포함된 구간을 자동으로 감지하여 하이라이트 후보를 제시합니다. 사용자의 수동 편집 없이도, 영상 속 말소리로부터 의미 있는 대화나 감정적인 반응이 담긴 부분을 추출하여 편집 효율을 향상시키는 것이 목표입니다.
...
|:--------------:|:-----------------------------------------------------------------------------------------|
| 💻 언어         | Python                                                                                   |
| 📚 주요 라이브러리 | PyQt5 (GUI), OpenCV (영상 처리), ffmpeg-python (썸네일 추출), SpeechRecognition (음성 인식), numpy 등 |
| 🎨 UI           | Qt Designer, PyQt5                                                                      |
| 💾 데이터 저장  | JSON 파일 기반                                                                           |
| ☁️ 기타         | 외부 AI/자막 변환 API 연동                                                               |
| 🖥️ 지원 OS      | Windows                                                                   |
